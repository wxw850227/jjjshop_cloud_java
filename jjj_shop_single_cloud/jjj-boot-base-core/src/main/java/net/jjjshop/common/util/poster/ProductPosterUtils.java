...
public class ProductPosterUtils {
    ...
    public String genePoster(Integer productId, String source, User user) throws Exception {
        ...
        if ("wx".equals(source)) {
            ...
            // h5码
            String url = String.format("%s/h5/pages/product/detail/detail?productId=%s&refereeId=%s&tenantId=%s", springBootJjjProperties.getServerIp(), productId, user.getUserId(), user.getTenantId());
        ...
        //Font productNameFont = new Font("宋体", Font.BOLD, 40);
    ...
    private String wrapText(String zh, FontMetrics metrics, int maxWidth) throws Exception {
    ...
    private String getPosterPath(User user, Product product, String source) {
        ...
        return tempPath + "/product/" + product.getTenantId() + "/" + source;
    }
    ...
    private String getPosterUrl(Product product, User user, String source) {
        return springBootJjjProperties.getResourceAccessUrl() + "temp/product/" + product.getTenantId() + "/" + source + "/" + this.getPosterName(user, product.getProductId()) + "?t=" + System.currentTimeMillis();
    }
}

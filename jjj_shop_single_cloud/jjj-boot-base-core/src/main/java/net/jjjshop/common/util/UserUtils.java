...
public class UserUtils {
     ...
     * @return
     */
    public List<UserGrade> getUsableGradeList(Long tenantId) {
        if (tenantId == null) {
        ...
        return userGradeService.list(new LambdaQueryWrapper<UserGrade>()
                .eq(UserGrade::getIsDelete, false).eq(UserGrade::getTenantId, tenantId)
    ...
    public void userGradeUpgrade(Integer userId) {
        ...
        // 获取所有等级
        List<UserGrade> list = this.getUsableGradeList(Long.valueOf(user.getTenantId()));
        if (list.size() == 0) {
            return;
        }

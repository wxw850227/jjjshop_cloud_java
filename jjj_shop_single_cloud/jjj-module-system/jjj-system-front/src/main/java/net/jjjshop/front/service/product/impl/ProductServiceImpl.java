...
public class ProductServiceImpl extends BaseServiceImpl<ProductMapper, Product> implements ProductService {
    ...
    public void setProductGradeMoney(ProductVo vo, User user) {
        ...
        if (discountRatio.compareTo(BigDecimal.ONE) < 0 && discountRatio.compareTo(BigDecimal.ZERO) > 0) {
            // 标记参与会员折扣
            ...
            //最小值0.01
            if(vo.getProductPrice().compareTo(new BigDecimal("0.01")) < 0){
                vo.setProductPrice(new BigDecimal("0.01"));
            }
            // 会员折扣价
            for (ProductSkuVo skuVo : vo.getSkuList()) {
                if (aloneGradeType == 20) {
                    ...
                    //最小值0.01
                    if(skuVo.getProductPrice().compareTo(new BigDecimal("0.01")) < 0){
                        skuVo.setProductPrice(new BigDecimal("0.01"));
                    }
                }
            }
        } else {

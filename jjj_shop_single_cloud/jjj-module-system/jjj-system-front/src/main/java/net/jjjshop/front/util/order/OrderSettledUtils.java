        ...
        for (ProductBuyVo product:this.productList) {
            ...
            if (discountRatio.compareTo(BigDecimal.ONE) < 0 && discountRatio.compareTo(BigDecimal.ZERO) > 0) {
                ...
                if (aloneGradeType == 20) {
                    ...
                    //最小值0.01
                    if(gradeProductPrice.compareTo(new BigDecimal("0.01")) < 0){
                        gradeProductPrice = new BigDecimal("0.01");
                    }
                    gradeTotalPrice = gradeProductPrice.multiply(new BigDecimal(product.getTotalNum()));
                }

...
public class UserServiceImpl extends BaseServiceImpl<UserMapper, User> implements UserService {
    ...
    public LoginUserTokenVo login(String mobile, String password) {
        ...
        App app = appService.getOne(new LambdaQueryWrapper<App>().eq(App::getTenantId,user.getTenantId())
                );
        if (app == null) {
    ...
    public boolean phoneRegister(PhoneRegisterParam phoneRegisterParam) {
        Long count = this.count(new LambdaQueryWrapper<User>().eq(User::getMobile, phoneRegisterParam.getMobile()));
        if (!(count > 0)) {
            User user = new User();

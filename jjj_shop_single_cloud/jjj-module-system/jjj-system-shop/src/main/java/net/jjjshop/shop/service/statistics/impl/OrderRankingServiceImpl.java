...
public class OrderRankingServiceImpl implements OrderRankingService {
    ...
    public JSONObject getProductData() throws ParseException {
        ...
        Long saleToday = productService.count(new LambdaQueryWrapper<Product>().eq(Product::getIsDelete, 0).eq(Product::getProductStatus, 10));
    ...
    public Map<String, Object> getOrderDataByDate(RankingParam rankingParam)  {
        ...
        for (Date t = startTime; t.before(endTime); t = DateUtil.offsetDay(t,1)) {
            ...
            }else if(rankingParam.getType().equals("refund")){
            ...
            days.add(day);
            data.add(json);
            saleMoney = saleMoney.add(totalMoney);
        }
        map.put("data", data);
        map.put("days", days);

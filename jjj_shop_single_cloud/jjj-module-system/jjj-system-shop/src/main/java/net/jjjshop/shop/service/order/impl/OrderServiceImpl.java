...
public class OrderServiceImpl extends BaseServiceImpl<OrderMapper, Order> implements OrderService {
    ...
    private List<OrderVo> getListAll(OrderPageParam orderPageParam) {
        ...
        List<OrderVo> result = this.list(wrapper).stream().map(e -> {
            ...
            vo.setDeliveryTypeText(DeliveryTypeEnum.getName(vo.getDeliveryType()));
            //如果是快递配送
            if (vo.getDeliveryType().equals(DeliveryTypeEnum.EXPRESS.getValue())) {
                ...
                //如果是自提配送
            } else if (vo.getDeliveryType().equals(DeliveryTypeEnum.EXTRACT.getValue())) {
    ...
    public Boolean verifyDelivery(Order order) {
        if (order.getPayStatus() != 20
                || !order.getDeliveryType().equals(DeliveryTypeEnum.EXPRESS.getValue())
     ...
     */
    @Override
    public Long getCount(String dataType) {
        LambdaQueryWrapper<Order> wrapper = new LambdaQueryWrapper<>();
    ...
    //获取订单统计数据
    @Override
    public BigDecimal getOrderData(String startDate, String endDate, String type)  {
        LambdaQueryWrapper<Order> wrapper = new LambdaQueryWrapper<>();
        //开始查询时间不为空
     ...
     */
    @Override
    public Long getReviewOrderTotal() {
    ...
    public Boolean verificationOrder(OrderExtractParam orderExtractParam) {
        ...
        if (order.getPayStatus() != 20
                || !order.getDeliveryType().equals(DeliveryTypeEnum.EXTRACT.getValue())
        ...
        orderList.add(order);
        orderUtils.complete(orderList, order.getTenantId());
        return update;
    }

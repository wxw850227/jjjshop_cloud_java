...
public class StoreOrderServiceImpl extends BaseServiceImpl<StoreOrderMapper, StoreOrder> implements StoreOrderService {
    ...
    public Paging<StoreOrderVo> getList(StoreOrderPageParam storeOrderPageParam) {
        ...
        if(CollectionUtil.isNotEmpty(storeOrderPageParam.getCreateTime()) && storeOrderPageParam.getCreateTime().size() == 2){
            String startDate = storeOrderPageParam.getCreateTime().get(0) + " 00:00:00";
            String endDate = storeOrderPageParam.getCreateTime().get(1) + " 23:59:59";
            wrapper.between(StoreOrder::getCreateTime,startDate,endDate);
        }
        //获取结果对象

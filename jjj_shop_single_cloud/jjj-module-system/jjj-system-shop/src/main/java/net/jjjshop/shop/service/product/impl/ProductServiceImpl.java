...
public class ProductServiceImpl extends BaseServiceImpl<ProductMapper, Product> implements ProductService {
    ...
    public Map<String,Object> getList(ProductPageParam productPageParam){
        ...
        //产品规格(10单规格 20多规格)
        if (productPageParam.getSpecType() != null && productPageParam.getSpecType() > 0){
            wrapper.eq(Product::getSpecType, productPageParam.getSpecType());
        }
    ...
    public boolean add(ProductParam productParam){
        //是否虚拟，0否1是 不是虚拟则运费模板不能为空
        if(productParam.getIsVirtual() == 0 && productParam.getDeliveryId() == 0){
    ...
    private void updateProductSpec(ProductParam productParam,  List<ProductSku> skuList)
    {
        // 活动商品不能删除，只能修改规格数据
        if (productParam.getSpecType() == 10) {
            ...
            sku.setProductSkuId(skus.get(0).getProductSkuId());
            sku.setSpecSkuId("0");
            productSkuService.updateById(sku);
        }else if (productParam.getSpecType() == 20) {
            // 修改商品sku
            this.updateSkuList(productParam, skuList);
        }
    }
     ...
     * @return
     */
    private void updateSkuList(ProductParam productParam, List<ProductSku> skuList){
    ...
    private void addProductSpec(ProductParam productParam, boolean isUpdate, List<ProductSku> skuList)
    {
        ...
        if (productParam.getSpecType() == 10) {
            //单规格 先删除
            productSkuService.remove(new LambdaQueryWrapper<ProductSku>().eq(ProductSku::getProductId, productParam.getProductId()));
            ProductSku sku = new ProductSku();
            BeanUtils.copyProperties(productParam.getSku(), sku);
            sku.setProductId(productParam.getProductId());
            sku.setSpecSkuId("0");
    ...
    private void setProductParams(ProductParam productParam, Product product){
        ...
        if(product.getProductPrice() != null && product.getProductPrice().compareTo(BigDecimal.ZERO) <= 0){
            throw new BusinessException("产品价格需大于0");
        }
    }
    ...
    public Map<String, Object> getBaseData(Integer productId, String scene){
        ...
        if(productId > 0){
            ...
            //详情视频id
            if(vo.getContentVideoId() != null && vo.getContentVideoId() > 0){
                vo.setContentVideo(uploadFileUtils.getFilePath(vo.getContentVideoId()));
            }
            //详情视频封面id
            if(vo.getContentPosterId() != null && vo.getContentPosterId() > 0){
                vo.setContentPoster(uploadFileUtils.getFilePath(vo.getContentPosterId()));
            }
            result.put("model", vo);
        }
        // 所有分类
    ...
    public boolean edit(ProductParam productParam){
        ...
        if(!productParam.getIsSpecLocked()){
            // 原sku
            ...
            List<ProductSku> skuList = productSkuService.list(new LambdaQueryWrapper<ProductSku>().eq(ProductSku::getProductId, productParam.getProductId()));
            // 保存商品规格,不能删除
            this.updateProductSpec(productParam, skuList);
        }
        // 保存商品图片
        this.addProductImages(productParam, product, 0);
        // 保存详情图片

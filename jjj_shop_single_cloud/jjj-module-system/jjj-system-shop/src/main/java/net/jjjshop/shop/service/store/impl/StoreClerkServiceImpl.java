...
public class StoreClerkServiceImpl extends BaseServiceImpl<StoreClerkMapper, StoreClerk> implements StoreClerkService {
    ...
    public Paging<StoreClerkVo> getList(StoreClerkPageParam storeClerkPageParam) {
        ...
        LambdaQueryWrapper<StoreClerk> wrapper = new LambdaQueryWrapper<StoreClerk>()
                .eq(StoreClerk::getIsDelete, 0).orderByDesc(StoreClerk::getCreateTime);
        ...
        //未删除的门店ID
        List<Integer> storeIds = storeService.list(new LambdaQueryWrapper<Store>()
                .eq(Store::getIsDelete, 0)
        ).stream().map(Store::getStoreId).collect(Collectors.toList());
        if(CollectionUtils.isEmpty(storeIds)){
            storeIds.add(0);
        }
        wrapper.in(StoreClerk::getStoreId, storeIds);
    ...
    public Boolean add(StoreClerkParam storeClerkParam) {
        ...
        Long count = this.count(new LambdaQueryWrapper<StoreClerk>()

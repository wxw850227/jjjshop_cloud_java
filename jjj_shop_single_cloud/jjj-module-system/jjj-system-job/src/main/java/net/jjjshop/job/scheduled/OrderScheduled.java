...
public class OrderScheduled {
    ...
    private void master(){
        ...
        for(App app:appList){
            // 获取商城交易设置
            JSONObject vo = settingUtils.getSetting(SettingEnum.TRADE.getKey(), Long.valueOf(app.getTenantId()));
            TradeVo tradeVo = JSONObject.toJavaObject(vo, TradeVo.class);
            // 已发货订单自动确认收货
            this.receive(tradeVo.getReceiveDays(), app.getTenantId());
            // 已完成订单结算
            this.settled(tradeVo.getRefundDays(), app.getTenantId());
        }
    }
     ...
     * 已发货订单自动确认收货
     */
    private void receive(Integer receiveDays, Integer tenantId){
        // 截止时间
        if (receiveDays < 1) {
            return;
        }
        ...
        List<Order> orderList = orderService.list(new LambdaQueryWrapper<Order>().eq(Order::getDeliveryStatus, 20)
                .eq(Order::getReceiptStatus, 10).eq(Order::getTenantId, tenantId)
        ...
        if (orderIds.size() > 0) {
            ...
            // 批量处理已完成的订单
            orderUtils.complete(orderList, tenantId);
        }
        // 记录日志
        if(orderIds.size() > 0){
     ...
     * 已完成订单结算
     */
    private void settled(Integer refundDays, Integer tenantId)
    {

	...
	<el-dialog title="编辑分类" v-model="dialogVisible" @close="dialogFormVisible" :close-on-click-modal="false"
		...
		<template #header="{ close, titleId, titleClass }">
			<div class="my-header d-b-c">
				<div :id="titleId" :class="titleClass" class="flex-1">编辑分类</div>
...
<script>
	...
	export default {
		...
		props: ["edit_category", "editform"],
		created() {
			this.dialogVisible = this.edit_category;
		...
		methods: {
			...
			addUser() {
				...
				self.$refs.form.validate((valid) => {
					if (valid) {
						self.loading = true;
						ProductApi.categoryEdit(params, true)
							.then((data) => {
								self.loading = false;
								ElMessage({
									message: "修改成功",

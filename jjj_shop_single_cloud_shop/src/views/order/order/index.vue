<template>
  <div class="user">
              ...
              <template v-if="!scope.row.isTopRow">
                ...
                <el-button
                  v-if="
                    ...
                    (!scope.row.assembleStatus ||
                      scope.row.assembleStatus == 20)
                  "
                  @click="expressClick(scope.row)"
                  text
                  type="primary"
                  size="small"
                ...
                <el-button
                  v-if="
                    ...
                    scope.row.orderStatus != 20 &&
                    scope.row.orderStatus != 21 &&
                    scope.row.deliveryType == 30
                  "
                  @click="virtualClick(scope.row)"
                  text
                  type="primary"
                  size="small"
                  v-auth="'/order/order/delivery'"
    ...
    <el-dialog title="批量发货说明" v-model="dialogBatchDelivery" width="30%">
      ...
      <div>
        2、在excel中填写物流公司、物流单号，然后点批量发货导入填好的excel文件
      </div>
      <div>
        3、请检查物流公司是否已经添加到系统，
    ...
    <express
      v-if="open_express"
      :open_express="open_express"
      :orderId="orderIds"
      @closeDialog="closeDialogFunc($event, 'express')"
    ></express>
    <Virtual
      v-if="open_virtual"
      :open_virtual="open_virtual"
      :orderId="orderIds"
      @closeDialog="closeDialogFunc($event, 'virtual')"
    ></Virtual>
  </div>
</template>
<script>
...
import qs from "qs";
import detailsFrom from "./orderDetails.vue";
import Cancel from "./dialog/cancel.vue";
import express from "./dialog/express.vue";
import Virtual from "./dialog/virtual.vue";
...
export default {
  ...
  methods: {
    ...
    closeDialogFunc(e, f) {
      ...
      if (f == "detail") {
        this.modals = e.openDialog;
        if (e.type == "success") {
          this.getData();
        }
      }
      if (f == "express") {
        this.open_express = e.openDialog;
        if (e.type == "success") {
          this.getData();
        }
      }
      if (f == "virtual") {
        this.open_virtual = e.openDialog;
        if (e.type == "success") {
          this.getData();
        }
      }
    ...
    onExport: function () {
      ...
      window.location.href =
        baseUrl + "/api/shop/order/operate/export?" + qs.stringify(Params);
    },
    // 显示批量发货说明
    showBatchDelivery() {
